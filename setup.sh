...
if ! grep -Exq "PATH=.*(~|$HOME)/SVN/internal/univention-support-info/usi-check.*" "$HOME/.bash_local"; then
	echo "PATH=\$PATH:~/SVN/internal/univention-support-info/usi-check" >>"$HOME/.bash_local"
fi
